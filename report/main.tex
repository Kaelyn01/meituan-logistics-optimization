...
The network consists of one CDC and five cities (A--E). Each city serves a set of retail stations with known daily demand and distances to the CDC. Two vehicle types are available: large trucks (capacity 200,000 boxes) and small trucks (capacity 80,000 boxes). The planning objective is to minimize the \emph{average daily total cost}, which comprises (i) inventory holding cost at the city and (ii) daily transportation cost, including both CDC-to-city long-haul and city-to-station local delivery.
...
\begin{figure}[htbp]
    ...
    \caption{(a) Logistics network topology: one CDC serves five cities (A--E). Each city is annotated with its optimal replenishment cycle $T^*$ and the required fleet size. (b) Optimal daily cost breakdown by city, split into storage cost (orange) and transport cost (navy).}
    \label{fig:topology}
\end{figure}
...
We distill four actionable recommendations for operations managers:
\begin{enumerate}
